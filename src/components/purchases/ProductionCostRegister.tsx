...
import { ArrowUpRight, CircleDollarSign, Pencil, Plus, ReceiptIndianRupee, Search, Trash2 } from 'lucide-react';
...
export default function ProductionCostRegister({ mode }: { mode: 'expenses' | 'payment-out' }) {
  ...
  const [editTarget, setEditTarget] = useState<CostRow | null>(null);
  ...
  async function deleteCost() {
    ...
    try { await api.delete(`/production-orders/${deleteTarget.orderId}/costs/${deleteTarget.id}`); setDeleteTarget(null); await loadRows(); }
    catch (deleteFailure: unknown) { setDeleteError(getApiError(deleteFailure, 'Could not delete this expense.')); }
...
function ExpenseModal({ orders, suppliers, expense, onClose, onSaved }: { orders: ProductionOrder[]; suppliers: Supplier[]; expense?: CostRow; onClose: () => void; onSaved: () => void }) {
  const editing = Boolean(expense);
  const [form, setForm] = useState({ orderId: expense?.orderId ?? '', category: (expense?.category ?? 'OTHER') as ProductionCostCategory, description: expense?.description ?? '', supplierId: expense?.supplier?.id ?? '', quantity: expense?.quantity ?? '1', rate: expense?.rate ?? '', paidAmount: expense?.paidAmount ?? '0', notes: expense?.notes ?? '' });
