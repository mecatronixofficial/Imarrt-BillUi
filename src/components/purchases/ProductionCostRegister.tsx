...
export default function ProductionCostRegister({
  mode,
}: {
  mode: "expenses" | "payment-out";
}) {
  ...
  const [category, setCategory] = useState<"ALL" | ProductionCostCategory>(
    "ALL",
  );
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      setRows(
        paymentMode
          ? costs.filter((cost) => Number(cost.paidAmount) > 0)
          : costs,
      );
    } catch (loadError: unknown) {
      setError(
        getApiError(loadError, `Could not load ${title.toLowerCase()}.`),
      );
    } finally {
      setLoading(false);
    }
  ...
  async function deleteCost() {
    ...
    try {
      await api.delete(
        `/production-orders/${deleteTarget.orderId}/costs/${deleteTarget.id}`,
      );
      toast.danger("Expense deleted");
  ...
  const total = rows.reduce(
    (sum, row) => sum + Number(paymentMode ? row.paidAmount : row.amount),
    0,
  );
  ...
  return (
    <>
      <PageHeader
        ...
        description={
          paymentMode
            ? "Review recorded supplier and production payments made from the business."
            : "Track material, labour, transport, and other project-linked business costs."
        }
        action={
          paymentMode ? (
            <Link
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-3">
        <Metric
          label={paymentMode ? "Payment records" : "Expense records"}
          value={rows.length.toLocaleString("en-IN")}
        />
        <Metric
          label={paymentMode ? "Total paid" : "Total expenses"}
          value={formatCurrency(total)}
        />
        <Metric label="Outstanding" value={formatCurrency(outstanding)} wide />
      </div>
        ...
        <div className="flex flex-col gap-3 border-b border-slate-100 p-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-[10px] text-slate-400">
              {filtered.length} project-based cost records
            </p>
          </div>
          ...
          <div className="flex w-full flex-col gap-2 sm:flex-row lg:w-auto">
            ...
            <div className="relative w-full sm:w-80">
              ...
              <input
                ...
                placeholder="Description, project or supplier..."
          ...
          <EmptyState
            ...
            description={
              query || category !== "ALL"
                ...
                : "Add your first project expense."
            }
          />
        ) : (
            ...
            <table className="w-full min-w-[1020px] text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((row) => {
                  const due = Math.max(
                    0,
                    Number(row.amount) - Number(row.paidAmount),
                  );
                  return (
                    <tr key={row.id} className="hover:bg-blue-50/30">
                      ...
                      <td className="px-4 py-3.5">
                        <Link
                          href={`/production?orderId=${row.orderId}`}
                      ...
                      <td className="px-4 py-3.5 text-slate-600">
                        {row.supplier?.name || "Not assigned"}
                      </td>
                      ...
                      <td
                        className={`px-4 py-3.5 text-right font-bold ${due ? "text-red-600" : "text-slate-400"}`}
                      >
                        {formatCurrency(due)}
                      </td>
                      {!paymentMode && (
      ...
      {deleteTarget && (
        <ConfirmDialog
          ...
          message={
            deleteError ||
            `Delete "${deleteTarget.description}" from ${deleteTarget.orderNumber}? Any payments recorded against it will be removed too. This cannot be undone.`
          }
          busy={deleting}
          onCancel={() => setDeleteTarget(null)}
          onConfirm={() => void deleteCost()}
        />
      )}
    </>
  );
...
function ExpenseModal({
  ...
  const [form, setForm] = useState({
    ...
    category: (expense?.category ?? "OTHER") as ProductionCostCategory,
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (expense)
        await api.patch(
          `/production-orders/${expense.orderId}/costs/${expense.id}`,
          {
        ...
        await api.post(`/production-orders/${form.orderId}/costs`, {
          category: form.category,
          description: form.description.trim(),
          ...
          rate: Number(form.rate),
          amount,
          paidAmount: Math.min(Number(form.paidAmount), amount),
          notes: form.notes.trim() || undefined,
        });
      (editing ? toast.warning : toast.success)(
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        {!orders.length && (
          <div className="rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800">
            Create a sales or production order before adding a project expense.
          </div>
        )}
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <Field label="Category *">
            <select
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  category: e.target.value as ProductionCostCategory,
                })
              }
            >
              {CATEGORIES.map((item) => (
          ...
          <Field label="Description *">
            <input
              ...
              placeholder="Transport, fabric, labour..."
            />
          </Field>
          ...
          <Field
            label={
              editing
                ? "Amount paid (use Payment-Out to change)"
                : "Amount paid"
            }
          >
            <input
          ...
          <div className="rounded-lg bg-blue-50 px-3 py-2 text-xs text-blue-800">
            Expense total
            <strong className="float-right text-base">
