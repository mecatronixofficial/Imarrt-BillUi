...
export default function Sidebar({ open = false, onClose, onHide, desktopHidden = false, role, userName }: SidebarProps) {
  ...
  const [workspaceBranches, setWorkspaceBranches] = useState<WorkspaceBranch[]>([]);
  const [workspaceBranchId, setWorkspaceBranchId] = useState('');
  ...
  const loadWorkspace = useCallback(() => {
    ...
    void Promise.all([getAllPages<Business>('/businesses'), api.get<WorkspaceBranch[]>('/workspace-branches')]).then(async ([{ data }, { data: groups }]) => {
      if (!active) return;
      setBusinesses(data);
      setWorkspaceBranches(groups);
      const stored = getActiveBusinessId();
      const storedWorkspaceBranch = getActiveWorkspaceBranchId();
      const selectedGroup = groups.find(({ id }) => id === storedWorkspaceBranch) ?? groups.find(({ businesses }) => businesses.some(({ id }) => id === stored)) ?? groups[0];
      const selected = selectedGroup?.businesses.some(({ id }) => id === stored) ? stored! : selectedGroup?.businesses[0]?.id ?? '';
      if (selected) setActiveBusinessId(selected);
      setSelectedBusinessId(selected);
      setWorkspaceBranchId(selectedGroup?.id ?? '');
      if (selectedGroup) setActiveWorkspaceBranchId(selectedGroup.id);
      if (!selected) return;

      const branchEntries = await Promise.all(data.map(async (business) => {
      ...
      const grouped = Object.fromEntries(branchEntries);
      setBranchesByBusiness(grouped);
      const availableBranches = grouped[selected] ?? [];
      if (!active) return;
  ...
  function handleLogicalBranchChange(id: string) {
    const group = workspaceBranches.find((branch) => branch.id === id);
    const preferredBusiness = group?.businesses[0];
    if (!preferredBusiness) return;
    const branch = branchesByBusiness[preferredBusiness.id]?.find(({ isActive }) => isActive);
    if (!branch) return;
  ...
  return (
    ...
    >
      <header className={clsx('relative flex h-16 shrink-0 items-center justify-between px-4 transition-all', collapsed && 'lg:h-24 lg:flex-col lg:justify-center lg:gap-2 lg:px-2 lg:py-2')}>
        ...
        }} className="flex items-center gap-2.5 rounded-lg text-left focus:outline-none" aria-label={collapsed ? 'Expand sidebar' : 'Minimize sidebar'} title={collapsed ? 'Expand sidebar' : 'Minimize sidebar'}>
        ...
        <div className={clsx('hidden items-center lg:flex', collapsed && 'gap-1')}>
          <button type="button" onClick={onHide} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-slate-400 transition hover:bg-white/10 hover:text-white focus:outline-none" aria-label="Hide sidebar" title="Hide sidebar">
        ...
        <button
          type="button"
          onClick={onClose}
          className="flex h-9 w-9 items-center justify-center rounded-lg text-slate-400 transition hover:bg-white/10 hover:text-white focus:outline-none lg:hidden"
      ...
      <div className={clsx('relative px-3 pb-2 transition-all', collapsed && 'lg:px-2')}>
        {activeBusiness ? (
          <div ref={businessMenuRef} className="relative z-30">
            ...
            >
              <p className="px-2.5 pb-1.5 pt-1 text-[9px] font-bold uppercase tracking-[0.16em] text-slate-500">Select branch</p>
              <div className="max-h-52 space-y-1 overflow-y-auto [scrollbar-width:thin]">
                {workspaceBranches.map((branch) => {
                  const selected = branch.id === workspaceBranchId;
                  return (
                    <button
                      type="button"
                      role="option"
                      ...
                      key={branch.id}
                      onClick={() => handleLogicalBranchChange(branch.id)}
                      className={`flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left outline-none transition ${selected ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-white/[0.07] hover:text-white'}`}
                    >
              ...
              <div className="mt-1 border-t border-white/10 pt-1">
                <p className="px-2.5 pb-1 pt-1.5 text-[9px] font-bold uppercase tracking-[0.16em] text-slate-500">Companies in branch</p>
                {availableBusinesses.map((business) => {
                  ...
                  return (
                    <button
                      ...
                      className={`mt-1 flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left outline-none transition ${selected ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-white/[0.07] hover:text-white'}`}
                    >
          ...
          {navItems.map(({ href, label, icon: Icon, children }) => {
              ...
              return (
                <div key={href}>
                  <button
                    ...
                    className={clsx(
                      'group relative flex h-11 w-full items-center gap-3 rounded-md px-3 text-left outline-none transition-all',
            ...
            return (
              <Link
                ...
                className={clsx(
                  'group relative flex h-11 items-center gap-3 rounded-md px-3 outline-none transition-all',
                  collapsed && 'lg:justify-center lg:gap-0 lg:px-2',
                  active
      ...
      <footer className={clsx('relative shrink-0 border-t border-white/10 p-3', collapsed && 'lg:p-2')}>
        ...
        <button
          ...
          className={clsx('group flex h-10 w-full items-center gap-2.5 rounded-lg px-2.5 text-xs font-medium text-slate-400 outline-none transition hover:bg-red-500/10 hover:text-red-300', collapsed && 'lg:justify-center lg:gap-0 lg:px-1')}
        >
