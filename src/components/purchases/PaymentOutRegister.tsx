...
export default function PaymentOutRegister() {
    ...
    try {
      const [payments, purchaseBills] = await Promise.all([
        ...
        getAllPages<BusinessDocument>("/documents", {
          params: { type: "PURCHASE_INVOICE" },
        }),
      ]);
      setRows(payments.data);
      setBills(
        purchaseBills.data.filter(
          (bill) =>
      ...
      setError(getApiError(loadError, "Could not load payment-out records."));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    return term
      ? rows.filter(({ document, method, reference }) =>
        [
  ...
  const outstanding = bills.reduce(
    (sum, bill) => sum + Number(bill.grandTotal) - Number(bill.paidAmount || 0),
    0,
  );
  ...
  return (
    <>
      <PageHeader
        ...
        title="Payment Out"
        description="Record money paid to suppliers against issued purchase bills."
        action={
          <button
            type="button"
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-3">
        <Metric label="Payments" value={rows.length.toLocaleString("en-IN")} />
        <Metric
          label="Outstanding payable"
          value={formatCurrency(outstanding)}
        />
        <Metric label="Total paid" value={formatCurrency(total)} wide />
      </div>
        ...
        <div className="flex flex-col gap-3 border-b border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-0.5 text-[10px] text-slate-400">
              Supplier payments for the active company and branch
            </p>
          </div>
          ...
          <div className="relative w-full sm:w-80">
            ...
            <input
              type="text"
              aria-label="Search payment out"
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="Bill, supplier, method, reference..."
          ...
          <EmptyState
            ...
            description={
              query
                ...
                : "Record a supplier payment against an issued purchase bill."
            }
          />
        ) : (
            ...
            <table className="w-full min-w-[920px] text-sm">
              <thead className="bg-slate-50 text-left text-[10px] uppercase tracking-wider text-slate-400">
                <tr>
                  ...
                  <th className="px-5 py-3">Purchase bill</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((row) => {
                  const paid =
                    Number(row.document.paidAmount || 0) >=
                    Number(row.document.grandTotal);
                  return (
                    <tr key={row.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-5 py-3.5">
                        ...
                        >
                          {row.document.referenceNumber ||
                            row.document.documentNumber}
                        </Link>
                      </td>
                      <td className="px-5 py-3.5 font-semibold text-slate-700">
                        {row.document.supplier?.name || "Not assigned"}
                      </td>
...
function PaymentOutModal({
  ...
  const [error, setError] = useState("");
  const selected = bills.find(({ id }) => id === documentId);
  const payable = selected
    ? Number(selected.grandTotal) - Number(selected.paidAmount || 0)
    : 0;
  async function submit(event: React.FormEvent) {
    event.preventDefault();
    if (!selected || saving) return;
    const value = Number(amount);
    if (!Number.isFinite(value) || value <= 0 || value > payable + 0.01)
      return setError("Enter an amount within the outstanding payable.");
    setSaving(true);
    setError("");
    try {
      await api.post(`/documents/${documentId}/payments`, {
        amount: value,
        method,
      ...
      toast.success("Payment recorded", {
        description: `${formatCurrency(value)} paid against ${selected.referenceNumber || selected.documentNumber}.`,
      });
      onSaved();
    } catch (saveError) {
  ...
  return (
    <Modal title="Record Payment Out" onClose={onClose} size="lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field label="Purchase bill *">
          <select
            ...
            value={documentId}
            onChange={(event) => {
              const bill = bills.find(({ id }) => id === event.target.value);
          ...
          >
            <option value="">Select outstanding purchase bill</option>
            {bills.map((bill) => (
              <option key={bill.id} value={bill.id}>
                {bill.referenceNumber || bill.documentNumber} ·{" "}
                {bill.supplier?.name || "Supplier"} · Due{" "}
                {formatCurrency(
                  Number(bill.grandTotal) - Number(bill.paidAmount || 0),
                )}
              </option>
        ...
        {selected && (
          <div className="rounded-xl bg-red-50 p-3 text-xs text-red-800">
            ...
            Previous payable: <strong>{formatCurrency(payable)}</strong>
            <span className="float-right">
              Remaining:{" "}
              <strong>
                {formatCurrency(Math.max(0, payable - (Number(amount) || 0)))}
              </strong>
            </span>
          </div>
        )}
